...
namespace Danganronpa_Another_Tool
{
    public partial class DRAT : Form
    {
        ...
        private void CloneDirectory(string OriginalDir, string TEMPFolder, string PAKType, int ConvertOrNotToConvert)
        {
            ...
            foreach (FileInfo fi in source.EnumerateFiles().Where(f => extensions.Contains(f.Extension.ToLower())).ToArray())
            {
                ...
                if (ConvertOrNotToConvert == 1 && !tabControl1.SelectedTab.Text.Contains("PSP") && (fi.Extension == ".png" || fi.Extension == ".tga"))
                {
                    ...
                    if (fi.Extension == ".png")
                    {
                        if (!ConvertFromPNGToTGA(fi.FullName, target.ToString()))
                        {
                            label5.Text = "Error!"; // Change the "Status" to "Error!".
                            return; //Don't keep trying to convert images if the needed program is missing.
                        }
                    ...
                    if (tabControl1.SelectedTab.Text.Contains("PSVITA"))
                    {
                        ...
                        if (!ConvertFromTGAToGXT(SingleImage, target.ToString()))
                        {
                            label5.Text = "Error!"; // Change the "Status" to "Error!".
                            return; //Don't keep trying to convert images if the needed program is missing.
                        }
        ...
        public static bool UseEXEToConvert(string FileEXE, string CodeLine)
        {
...
                             In this case all of them are folders, so there is no need to memorize it. */
        ...
        private bool ExtractPAK(string PAKFileAddress, string DestinationDir, int ToPNG)
        {
            if (Directory.Exists(DestinationDir) == false)
                Directory.CreateDirectory(DestinationDir);
            ...
            using (OpenFileDialog IMAGES = new OpenFileDialog())
            {
                ...
                if (IMAGES.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (string SingleImage in IMAGES.FileNames)
                    {
                        ...
                        if (ExtesionIMG == ".tga")
                        {
                            if (!ConvertFromTGAToPNG(SingleImage, DestinationDir))
                            {
                                label5.Text = "Error!"; // Change the "Status" to "Error!".
                                return; //Don't keep trying to convert images if the needed program is missing.
                            }
                        ...
                        {
                            if (!ConvertGxtBtxToPNG(SingleImage, DestinationDir))
                            {
                                label5.Text = "Error!"; // Change the "Status" to "Error!".
                                return; //Don't keep trying to convert images if the needed program is missing.
                            }
                        ...
                        {
                            if (!ConvertFromGIMToPNG(SingleImage, DestinationDir))
                            {
                                label5.Text = "Error!"; // Change the "Status" to "Error!".
                                return; //Don't keep trying to convert images if the needed program is missing.
                            }
            ...
            using (OpenFileDialog IMAGES = new OpenFileDialog())
            {
                ...
                if (IMAGES.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (string SingleImage in IMAGES.FileNames)
                    {
                        ...
                        if (!ConvertFromPNGToTGA(ImageToConvert, DestinationDir))
                        {
                            label5.Text = "Error!"; // Change the "Status" to "Error!".
                            return; //Don't keep trying to convert images if the needed program is missing.
                        }
            ...
            using (OpenFileDialog IMAGES = new OpenFileDialog())
            {
                ...
                if (IMAGES.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (string SingleImage in IMAGES.FileNames)
                    {
                        ...
                        if (!ConvertFromTGAToGXT(ImageToConvert, DestinationDir))
                        {
                            label5.Text = "Error!"; // Change the "Status" to "Error!".
                            return; //Don't keep trying to convert images if the needed program is missing.
                        }
